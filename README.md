...
- 수급, 거래대금, 등락률, 시간외 가격을 합산한 관심 신호 리포트
- 관심 신호 상위 종목의 오전/오후 가격 기준 실적 리포트
...
| `신호 오늘` | 수급, 거래대금, 등락률, 시간외 가격을 합산한 시장별 관심 신호 |
| `신호 삼성전자` | 종목명 기준 관심 신호 상세 |
| `신호 005930` | 종목 코드 기준 관심 신호 상세 |
| `오전실적 오늘` | 관심 신호 상위 종목의 09:00 → 11:30 가격 성과 |
| `오후실적 오늘` | 관심 신호 상위 종목의 12:00 → 15:30 가격 성과 |
...
슬래시가 붙은 텔레그램 명령어(`/도움말`, `/연기금 오늘`)와 슬래시 없는 일반 텍스트 입력(`도움말`, `연기금 오늘`)을 모두 지원합니다. 기존 영문 명령어(`start`, `help`, `pension today`, `stock 005930`, `afterhours up`, `signal today`, `morning today`, `afternoon today`)도 함께 지원합니다.
...
- `/신호 오늘`은 예측이나 추천이 아니라 연기금등 순매수, 거래대금 대비 비중, 거래대금, 당일 등락률, 시간외 가격을 합산한 참고 점수입니다.
- `/오전실적 오늘`, `/오후실적 오늘`은 연기금등 시간대별 수급이 아니라 네이버 금융 분봉 가격 기준 성과입니다.
